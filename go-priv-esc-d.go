...
// Function to get the PEB address of a process
func getPEBAddress(processHandle syscall.Handle) (uintptr, error) {
	...
	if status != 0 {
		return 0, fmt.Errorf("error calling NtQueryInformationProcess: %x", status)
	}
	return processBasicInfo.PebBaseAddress, nil
}

// Function to get the parent process ID of a process
func getParentProcessID(processHandle syscall.Handle) (uint32, error) {
	var processBasicInfo PROCESS_BASIC_INFORMATION
	var returnLength uintptr
	status, _, _ := procNtQueryInformationProcess.Call(uintptr(processHandle), 0, uintptr(unsafe.Pointer(&processBasicInfo)), unsafe.Sizeof(processBasicInfo), uintptr(unsafe.Pointer(&returnLength)))
	if status != 0 {
	...
	return uint32(processBasicInfo.Reserved3), nil
}

// Function to get the process access token
func getProcessToken(processHandle syscall.Handle) (syscall.Token, error) {
	var token syscall.Token
	status, _, _ := procOpenProcessToken.Call(uintptr(processHandle), syscall.TOKEN_QUERY, uintptr(unsafe.Pointer(&token)))
	if status == 0 {
		return 0, fmt.Errorf("error calling OpenProcessToken: %x", status)
	}
	return token, nil
}
...
func main() {
	...
	pebAddress, err := getPEBAddress(processHandle)
	if err != nil {
		fmt.Println("Error getting PEB address:", err)
	} else {
		fmt.Println("PEB address:", pebAddress)
	}

	// Get the parent process ID of the process
	parentProcessID, err := getParentProcessID(processHandle)
	if err != nil {
		fmt.Println("Error getting parent process ID:", err)
	} else {
		fmt.Println("Parent Process ID:", parentProcessID)
	}

	// Get the process access token
	processToken, err := getProcessToken(processHandle)
	if err != nil {
		fmt.Println("Error getting process token:", err)
	} else {
		fmt.Println("Process token:", processToken)
	}
